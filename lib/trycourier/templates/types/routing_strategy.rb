...
module Courier
  class Templates
    class RoutingStrategy
      ...
      # @param method [ROUTING_STRATEGY_METHOD] The method for selecting channels to send the message with. Value can be either 'single' or 'all'. If not provided will default to 'single'
      ...
      def initialize(method:, channels:, additional_properties: nil)
        # @type [ROUTING_STRATEGY_METHOD] The method for selecting channels to send the message with. Value can be either 'single' or 'all'. If not provided will default to 'single'
        @method = method
      ...
      def self.from_json(json_object:)
        ...
        method = Templates::ROUTING_STRATEGY_METHOD.key(parsed_json["method"]) || parsed_json["method"]
      ...
      # @return [JSON]
      def to_json(*_args)
        { "method": Templates::ROUTING_STRATEGY_METHOD[@method] || @method, "channels": @channels }.to_json
      end
      ...
      def self.validate_raw(obj:)
        obj.method.is_a?(Templates::ROUTING_STRATEGY_METHOD) != false || raise("Passed value for field obj.method is not the expected type, validation failed.")
