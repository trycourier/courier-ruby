  ...
  class Send
    class Preference
      ...
      def self.from_json(json_object:)
        ...
        source = Send::CHANNEL_SOURCE.key(parsed_json["source"]) || parsed_json["source"]
      ...
      def to_json(*_args)
        {
          ...
          "rules": @rules,
          "channel_preferences": @channel_preferences,
          "source": Send::CHANNEL_SOURCE[@source] || @source
        }.to_json
      end
      ...
      def self.validate_raw(obj:)
        ...
        obj.source&.is_a?(Send::CHANNEL_SOURCE) != false || raise("Passed value for field obj.source is not the expected type, validation failed.")
      end
    end
  end
end
