...
module Courier
  class Bulk
    class BulkMessageUserResponse
      ...
      # @param status [BULK_JOB_USER_STATUS]
      ...
      def self.from_json(json_object:)
        ...
        status = Bulk::BULK_JOB_USER_STATUS.key(parsed_json["status"]) || parsed_json["status"]
      ...
      def to_json(*_args)
        {
          "status": Bulk::BULK_JOB_USER_STATUS[@status] || @status,
      ...
      def self.validate_raw(obj:)
        obj.status.is_a?(Bulk::BULK_JOB_USER_STATUS) != false || raise("Passed value for field obj.status is not the expected type, validation failed.")
