  ...
  class Bulk
    class JobDetails
      ...
      def self.from_json(json_object:)
        ...
        status = Bulk::BULK_JOB_STATUS.key(parsed_json["status"]) || parsed_json["status"]
      ...
      def to_json(*_args)
        {
          ...
          "failures": @failures,
          "received": @received,
          "status": Bulk::BULK_JOB_STATUS[@status] || @status
        }.to_json
      end
      ...
      def self.validate_raw(obj:)
        ...
        obj.status.is_a?(Bulk::BULK_JOB_STATUS) != false || raise("Passed value for field obj.status is not the expected type, validation failed.")
      end
    end
  end
end
