...
require_relative "types/user_profile_patch"
...
module Courier
  class ProfilesClient
    ...
    # @param user_id [String] A unique identifier representing the user associated with the requested profile.
    # @param request [Array<Hash>] Request of type Array<Profiles::UserProfilePatch>, as a Hash
    #   * :op (String)
    #   * :path (String)
    #   * :value (String)
    # @param request_options [RequestOptions]
    # @return [Void]
    def merge_profile(user_id:, request:, request_options: nil)
      @request_client.conn.patch("/profiles/#{user_id}") do |req|
        req.options.timeout = request_options.timeout_in_seconds unless request_options&.timeout_in_seconds.nil?
        unless request_options&.authorization_token.nil?
          req.headers["Authorization"] =
            request_options.authorization_token
