      ...
      class PooledNetRequester
        ...
        class << self
          ...
          def calibrate_socket_timeout(conn, deadline)
            timeout = deadline - Courier::Internal::Util.monotonic_secs
        ...
        private def with_pool(url, deadline:, &blk)
          origin = Courier::Internal::Util.uri_origin(url)
          timeout = deadline - Courier::Internal::Util.monotonic_secs
          pool =
        ...
        def execute(request)
          ...
          enum = Enumerator.new do |y|
            ...
            raise Courier::Errors::APITimeoutError.new(url: url, request: req)
          rescue StandardError
            raise Courier::Errors::APIConnectionError.new(url: url, request: req)
          end
          # rubocop:enable Metrics/BlockLength

          _, response = enum.next
          body = Courier::Internal::Util.fused_enum(enum, external: true) do
            finished = true
            loop { enum.next }
          ensure
            closing&.call
