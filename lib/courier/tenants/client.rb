...
require_relative "types/subscription_topic_new"
require "async"
...
module Courier
  class TenantsClient
    ...
    # @param parent_tenant_id [String] Filter the list of tenants by parent_id
    # @param limit [Integer] The number of tenants to return
    #   (defaults to 20, maximum value of 100)
    # @param cursor [String] Continue the pagination with the next cursor
    # @param request_options [RequestOptions]
    # @return [Tenants::TenantListResponse]
    def list(parent_tenant_id: nil, limit: nil, cursor: nil, request_options: nil)
    ...
    # @param request [Hash] Request of type Tenants::SubscriptionTopicNew, as a Hash
    #   * :status (Tenants::SubscriptionTopicStatus)
    #   * :has_custom_routing (Boolean)
    #   * :custom_routing (Array<Commons::ChannelClassification>)
    # @param request_options [RequestOptions]
    # @return [Void]
    def create_or_replace_default_preferences_for_topic(tenant_id:, topic_id:, request:, request_options: nil)
      @request_client.conn.put("/tenants/#{tenant_id}/default_preferences/items/#{topic_id}") do |req|
        req.options.timeout = request_options.timeout_in_seconds unless request_options&.timeout_in_seconds.nil?
        unless request_options&.authorization_token.nil?
          req.headers["Authorization"] =
            request_options.authorization_token
    ...
    # @param request_options [RequestOptions]
    # @return [Void]
    def remove_default_preferences_for_topic(tenant_id:, topic_id:, request_options: nil)
      @request_client.conn.delete("/tenants/#{tenant_id}/default_preferences/items/#{topic_id}") do |req|
        req.options.timeout = request_options.timeout_in_seconds unless request_options&.timeout_in_seconds.nil?
        unless request_options&.authorization_token.nil?
          req.headers["Authorization"] =
            request_options.authorization_token
