...
class TrycourierTest < Minitest::Test
  ...
  def test_raises_on_missing_non_nullable_opts
    e = assert_raises(ArgumentError) do
      Trycourier::Client.new
    end
    assert_match(/is required/, e.message)
  end

  def test_client_default_request_default_retry_attempts
    stub_request(:post, "http://localhost/send").to_return_json(status: 500, body: {})

    courier = Trycourier::Client.new(base_url: "http://localhost", api_key: "My API Key")

    assert_raises(Trycourier::Errors::InternalServerError) do
  ...
  def test_client_given_request_default_retry_attempts
    ...
    courier = Trycourier::Client.new(base_url: "http://localhost", api_key: "My API Key", max_retries: 3)
  ...
  def test_client_retry_after_seconds
    stub_request(:post, "http://localhost/send").to_return_json(
      status: 500,
      headers: {"retry-after" => "1.3"},
      body: {}
    )

    courier = Trycourier::Client.new(base_url: "http://localhost", api_key: "My API Key", max_retries: 1)
  ...
  def test_client_redirect_307
    ...
    assert_raises(Trycourier::Errors::APIConnectionError) do
    ...
    assert_requested(:any, "http://localhost/redirected", times: Trycourier::Client::MAX_REDIRECTS) do
  ...
  def test_client_redirect_303
    ...
    assert_requested(:get, "http://localhost/redirected", times: Trycourier::Client::MAX_REDIRECTS) do
  ...
  def test_client_redirect_auth_strip_cross_origin
    ...
    assert_requested(:any, "https://example.com/redirected", times: Trycourier::Client::MAX_REDIRECTS) do
      headers = _1.headers.keys.map(&:downcase)
  ...
  def test_default_headers
    stub_request(:post, "http://localhost/send").to_return_json(status: 200, body: {})
